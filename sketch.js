(() => {
    ...
    let Ps = [...Array(8)].map((e, i) => ({
        ...
        variant: variants[0 | (Math.random() * variants.length)],
        ...
        reset() {
            ...
            this.variant = variants[0 | (Math.random() * variants.length)];
            this.xv = 0; this.yv = 0;
            this.fade = 0;
        },
        update(dt) {
    ...
    function animate(t) {
        ...
        for (let p of Ps) {
            ...
            // ctx.closePath();
            // ctx.fill();
            ctx.font = `${1.5 * p.r}px "Fira Code"`;
            ctx.save();
            ctx.translate(c.width * (p.x + 0.5), c.height * (p.y + 0.5));
            ctx.rotate(p.a);
            ctx.fillText(p.variant, 0, 0);
            ctx.restore();
            p.update(dt / 200000);
        }
